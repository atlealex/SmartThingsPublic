...
module.exports = {
  /**
   * Only returns people who are home or in a named zone - plain "away"
   * people are left out of the widget entirely. Sorted by the "sortOrder"
   * device setting (lower first), then by name for people sharing the
   * same number.
   */
  async getPeople({ homey }) {
    ...
    const visibleDevices = driver.getDevices()
      .filter((device) => device.getCapabilityValue('home') === true || Boolean(device.getCapabilityValue('zone')))
      .sort((a, b) => {
